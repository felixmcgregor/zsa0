...
class TestNet(nn.Module):
    ...
    def __init__(
        self, conv_filter_size=32, n_residual_blocks=1
    ):  # Use exact config from main.py
        super().__init__()
...
def test_network():
    ...
    try:
        ...
        assert policy.shape == (batch_size, N_COLS), (
            f"Policy shape mismatch: {policy.shape} != ({batch_size}, {N_COLS})"
        )
        assert q_values.shape == (batch_size, 2), (
            f"Q values shape mismatch: {q_values.shape} != ({batch_size}, 2)"
        )
